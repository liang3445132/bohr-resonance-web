...
这是由原 Python 项目完整迁移得到的 HTTP 网站。打开浏览器即可完成 Excel 上传、数据校验、参数计算、曲线预览与结果下载；原始文件不会被修改。
...
测试使用项目自带的两份示例 Excel，验证两种分析、关键数值和 PNG 生成。
...
- `damping_analysis/`、`forced_analysis/`：保留的原分析模块。
